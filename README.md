# 👋 Rafael Lisboa

Software Developer focado em aplicações web modernas, backend estruturado e desenvolvimento multiplataforma.

Construo soluções digitais simples, escaláveis e sustentáveis, com atenção à organização de código, performance e boas práticas.

---

## 💻 Ambiente de Desenvolvimento

- Dell Inspiron 14 Plus  
- Snapdragon X Plus (ARM64)  
- 16GB RAM  
- Windows 11 ARM  
- Desenvolvimento e testes em arquitetura ARM  

---

## 🚀 Stack Principal

### 🎨 Frontend & Desktop
<div style="display: inline_block">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/electron/electron-original.svg">
</div>

- Desenvolvimento de aplicações desktop com Electron  
- Desenvolvimento de extensões para Google Chrome  

---

### ⚙️ Backend
<div style="display: inline_block">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/laravel/laravel-original.svg">
</div>

---

### 🗄️ Banco de Dados
<div style="display: inline_block">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg">
</div>

MySQL • MariaDB • PostgreSQL

---

### 🐧 Ambiente & Infra
<div style="display: inline_block">
  <img height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg">
</div>

---

## 🧩 Experiência Complementar

- Estruturação de aplicações com Laravel  
- APIs RESTful e integrações  
- Evolution API  
- Colify  
- Autenticação JWT  
- Deploy e configuração de ambientes Linux  
- Versionamento com Git  
- Uso de IA no fluxo de desenvolvimento (Claude Code)  

---

## 🎯 Foco Atual

- Desenvolvimento Full Stack com React e Node  
- Aplicações desktop multiplataforma com Electron  
- Desenvolvimento de extensões Chrome  
- Backend estruturado com Laravel e Python  
- Modelagem e otimização de banco de dados  
- Arquitetura limpa e boas práticas  

---

## 📫 Contato

📧 lisboa.codes@gmail.com  
📷 Instagram: @lisboacodes  
---

☕ Café, código e constância.
